		...
		var bonusRate = function () {
			return{				
		        ...
		        add:function () {
		            	flag = true; 
		    	  if(flag){
		        	document.getElementById("directlyUnderSaveButton").setAttribute("disabled", false);
			            	$.ajax({
			                    url:'directly_under_manager/add_directly_under',
			                    type:'post',
			                    dataType:'json',
			                    ...
			                    success:function(data){
			                        if(data){
			                		    ...
			                		    document.getElementById("directlyUnderSaveButton").removeAttribute("disabled");
			                        }
			                    },
			                    error:function(){
			                        $.alert({
			 ...
			 $(document).ready(function() {
			        ...
			        var InputsUnderList  = $("#InputsUnderList"); //Input boxes wrapper ID
			        var AddButton    = $("#underTable"); //Add button ID
			        var x = InputsUnderList.length; //initlal text box count
			        ...
			        $(AddButton).click(function (e) //on add input button click
			          {
			            if(x <= MaxInputs) //maximum input box allowed
			            {
							...
							rowId++;
			                //add input box
			                $(InputsUnderList).append('<tr id=tr'+ FieldCount +'>'
			                		+'<td> 总价值佣金FYC（元）</td>'
		   ...
		   //删除
			function delRowNurture(id) {
				...
				/* 查询  */
				$.ajax({
					url : "directly_under_manager/check_directly_under_size",
					type : "post",
					dataType : "json",
					data : {
						id : id
					...
					success : function(size) {
						 if (size >1) {
							 /* 删除 */
								$.ajax({
									url : "directly_under_manager/update_directly_under_status",
									type : "post",
									dataType : "json",
									data : {
										id : id
...
<div>
 <a href="#" rel="external nofollow" id="underTable" class="btn btn-info">添加</a> 

	<table id="InputsUnderList" border="1" style="display：table-row; table-layout:fixed;width:95%;"  >
		  <tr style="background: silver;">
			<th width="20%"  style="text-align:center;">设置项</th>
		 	<th width="50%"  style="text-align:center;">规则</th>
		    <th width="15%"  style="text-align:center;">发放系数（%）</th>
		     <th width="12%"  style="text-align:center;">操作</th>
		  </tr>
		   <c:forEach items="${directlyUnderManager}" var="base"> 
		 <tr >
		  ...
		  <td><a href="#"  rel="external nofollow" class="removeclass"><input  type="button" value="删除" onclick="delRowNurture('${base.id}');" ></a></td>
            ...
            <input type="reset" name="reset" style="display: none;" />
                     <button type="button" id="underBianji" class="btn btn-primary" >编辑</button>
               <button id="directlyUnderSaveButton" type="button" onclick="bonusRate.add()" class="btn btn-primary">保存</button>
            </div>
            </form>
          </div>
</body>
