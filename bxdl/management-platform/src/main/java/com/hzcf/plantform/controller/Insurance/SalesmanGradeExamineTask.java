 ...
 */
@Component
public class SalesmanGradeExamineTask {
   ...
   // @Scheduled(cron = "0 0 0 25 * ? ")
    public void examineSalesmanGrade(){
        ...
        List<InsuranceSalesInfo> gcqkzList = insuranceSalesInfoClient.insuranceSalesList(param);
        gcqkzList.forEach(a -> {
        ...
        List<InsuranceSalesInfo> kzList = insuranceSalesInfoClient.insuranceSalesList(param);
        kzList.forEach(a -> {
            int m = this.takeOfficeMonth(a.getJoinDate());
            if (m >= 3 && !promoteInsuranceSalesInfo(a)) {
                if (1 == month || 4 == month || 7 == month || 10 == month) {
        ...
        List<InsuranceSalesInfo> gcqczList = insuranceSalesInfoClient.insuranceSalesList(param);
        gcqczList.forEach(a -> {
            int m = 0;
            String moveTime = insuranceSalesInfoClient.findEmploymentPeriod(a.getInsuranceSalesId(), a.getSalesGradeId());
        ...
        List<InsuranceSalesInfo> czList = insuranceSalesInfoClient.insuranceSalesList(param);
        czList.forEach(a -> {
            int m = 0;
            String moveTime = insuranceSalesInfoClient.findEmploymentPeriod(a.getInsuranceSalesId(), a.getSalesGradeId());
            if(StringUtils.isNotBlank(moveTime)){
        ...
        List<InsuranceSalesInfo> gcqbzList = insuranceSalesInfoClient.insuranceSalesList(param);
        ...
        List<InsuranceSalesInfo> bzList = insuranceSalesInfoClient.insuranceSalesList(param);
        bzList.forEach(a -> {
            int m = 0;
            String moveTime = insuranceSalesInfoClient.findEmploymentPeriod(a.getInsuranceSalesId(), a.getSalesGradeId());
            if(StringUtils.isNotBlank(moveTime)){
    ...
    private boolean promoteInsuranceSalesInfo(InsuranceSalesInfo info) {
        ...
        // 查询考核结果
        SalesAssess salesAssess = insuranceSalesInfoClient.findNewSalesAssess(info.getInsuranceSalesId(), info.getSalesGradeId());
        if (null == salesAssess) {
            return result;
        }
        if (GCQKZ == salesAssess.getSalesGradeId()) {
            ...
            if (YES.equals(info.getCertificateStatus()) && YES.equals(salesAssess.getCondition1())) {
                double fyc = insuranceSalesInfoClient.getFYC(info.getInsuranceSalesId(), 3, null);
                if (fyc >= 1500) {
                    result = true;
            ...
            if (YES.equals(salesAssess.getCondition8()) && YES.equals(salesAssess.getCondition3())) {
                double fyc = insuranceSalesInfoClient.getFYC(info.getInsuranceSalesId(), 1, info.getJoinDate());
                if (fyc >= 1000) {
                    // 推荐科长人数
                    int tjCount = insuranceSalesInfoClient.queryBTJCount(info.getInsuranceSalesId());
                    if (tjCount >= 6) {
                        double sum = 0;
                        // 通过查询增员id及季度fyc
                        List<Long> btjSalesIds = insuranceSalesInfoClient.queryTJSubordinateForKZ(info.getInsuranceSalesId());
                        if (!CollectionUtils.isEmpty(btjSalesIds)) {
                           ...
                           /* btjSalesIds.forEach(a -> {
                                bigDecimal.add(BigDecimal.valueOf(insuranceSalesInfoClient.getFYC(a, 3, null)));
                            });*/
                            for (Long a:btjSalesIds) {
                               bigDecimal = bigDecimal.add(BigDecimal.valueOf(insuranceSalesInfoClient.getFYC(a, 3, null)));
                            }
                            fyc = insuranceSalesInfoClient.getFYC(info.getInsuranceSalesId(), 3, null);
            ...
            if (YES.equals(salesAssess.getCondition8()) && YES.equals(salesAssess.getCondition3())) {
                double fyc = insuranceSalesInfoClient.getFYC(info.getInsuranceSalesId(), 3, info.getJoinDate());
                if (fyc >= 1000) {
                    // 育成处长数量
                    int ycCount = insuranceSalesInfoClient.queryBYCCount(info.getInsuranceSalesId(), 3);
                    if (ycCount >= 3) {
                        double sum = 0;
                        // 查询直辖组员工id
                        ...
                        if (!CollectionUtils.isEmpty(subordinateIds)) {
                            ...
                            double v = insuranceSalesInfoClient.getFYC(info.getInsuranceSalesId(), 3, null);
...
//        }
        ...
        if (GCQKZ == info.getSalesGradeId() || KZ == info.getSalesGradeId()) {
            ...
            if (!CollectionUtils.isEmpty(subordinateIds)) {
                ...
                for (Long id : subordinateIds) {
                    double subordinateFyc = insuranceSalesInfoClient.getFYC(id, 3, null);
    ...
    private void keepBZAssess(InsuranceSalesInfo info, int month) {
        double fyc = insuranceSalesInfoClient.getFYC(info.getInsuranceSalesId(), 6, info.getJoinDate());
        if (fyc < 1000) {
            this.saveSalesMoveLog(info, 2);
        }
        ...
        // 之前半年达成处长人数
        int count = insuranceSalesInfoClient.queryDutyCount(info.getInsuranceSalesId(), 3, startTime, endTime);
        if (count < 2) {
            this.saveSalesMoveLog(info, 2);
        }
        double sum = 0;
        ...
        if (!CollectionUtils.isEmpty(subordinateIds)) {
            ...
            for (Object id : subordinateIds) {
                BigDecimal decimal = BigDecimal.valueOf(insuranceSalesInfoClient.getFYC(Long.parseLong(String.valueOf(id)), 6, null));
    ...
    private void saveSalesMoveLog(InsuranceSalesInfo info, int state) {
        if (GCQKZ == info.getSalesGradeId() && 2 == state) {
            ...
            insuranceSalesInfoClient.insertMove(logs);
        }
    }

    private List<Long> getSubordinate(List<Long> subordinateSalesIds, Long salesId, Long salesGradeId) {
        List<Long> ids = insuranceSalesInfoClient.querySubordinate(salesId, salesGradeId);
    ...
    private List<Long> getBeRecommendedKZSalesId(List<Long> btjSalesIds, Long tjSalesId) {
        List<Long> ids = insuranceSalesInfoClient.queryBeRecommendedKZBySalesId(tjSalesId);
        if (!CollectionUtils.isEmpty(ids)) {
            for (Long id : ids) {
