 ...
 * @package:com.huizhongcf.lender.admin.util
 * @Class:HttpRequestUtil
 * @date:2019年1月8日下午3:48:21
 * @author:sxm
 */
    ...
    public static String sendPostForSendMsg(String url, String param) throws Exception {
        ...
        try {
            ...
            conn.setRequestProperty("user-agent",
                    "Mozilla/4.0 (compatible; MSIE 6.0; Windows NT 5.1;SV1)");
            conn.setRequestProperty("Content-Type", "application/json;charset=utf-8");
            conn.setConnectTimeout(6000000);//设置连接主机超时时间（单位：毫秒）
            conn.setReadTimeout(6000000);//设置从主机读取数据超时时间（单位：毫秒）
            // 发送POST请求必须设置如下两行
            conn.setDoOutput(true);
            conn.setDoInput(true);
            // 获取URLConnection对象对应的输出流
